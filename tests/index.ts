...
Meteor.isServer &&
  Tinytest.addAsync("rpc - example", async function (test) {
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - text", async function (test) {
    const id = new Date().toISOString();
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - sum", async function (test) {
    const id = new Date().toISOString();
    const test1 = createMethod(
      `${id}.sum`,
      z.array(z.number(), z.number()),
      ([a1, a2]) => a1 + a2
    );
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - identity", async function (test) {
    const id = new Date().toISOString();
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - join str", async function (test) {
    const id = new Date().toISOString();
    const test1 = createMethod(
      `${id}.sum`,
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - err", async function (test) {
    const id = new Date().toISOString();
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - add resolver later", async function (test) {
    const id = new Date().toISOString();
...
Meteor.isServer &&
  Tinytest.addAsync(
    "rpc - add resolver later using zod",
    async function (test) {
      const id = new Date().toISOString();
      const math = z.object({ n1: z.number(), n2: z.number() });
...
Meteor.isServer &&
  Tinytest.addAsync("rpc - full test", async function (test) {
    const nameValidator = z.object({ first: z.string(), last: z.string() });
    const getFullName = createMethod(
      "some",
      nameValidator,
      async ({ first, last }) => {
